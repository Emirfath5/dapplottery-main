...
const Result = ({ jackpot, participants, result }) => {
  ...
  return (
    <div className="mx-auto py-16 bg-fuchsia-100 space-y-2">
      <div className="flex flex-col items-center justify-center text-center py-10">
        <h1 className="text-2xl font-bold pb-4">Lottery Result</h1>
        <p className="text-lg text-fuchsia-600 font-semibold capitalize">{jackpot?.title}</p>
        <p className="text-sm text-fuchsia-500 w-full sm:w-2/3">{jackpot?.description}</p>
        <p className="text-sm text-fuchsia-500 w-full sm:w-2/3">
          Result for{' '}
      ...
      <div className="flex flex-col justify-center items-center space-y-4">
        ...
        <div className="flex justify-center items-center space-x-2">
          ...
          <Link
            ...
            className="flex flex-nowrap border py-2 px-4 rounded-full bg-[#cf16d9]
      ...
      <div className="flex flex-col-reverse sm:flex-row ">
        ...
        p-5 rounded-md"
        >
          <h4 className="text-2xl font-bold text-fuchsia-700 text-center">Winners & Lossers</h4>

          <div className="space-y-2 max-h-80 overflow-y-auto">
            {participants?.map((participant, i) => (
              <div
                key={i}
                className="flex justify-start items-center border-b border-fuchsia-100 py-2 space-x-2"
              >
                <Identicon size={30} string={i} className="rounded-full h-12 w-12" />
                <div className="flex justify-center items-center space-x-2 text-sm">
                  <p className="font-semibold text-lg text-fuchsia-500">
                    {truncate(participant.account, 4, 4, 11)}
                  </p>
                  <p className="text-fuchsia-500">{participant.lotteryNumber}</p>
